...
class Editor:
    ...
    def render(self, width: int) -> list[str]:
        ...
        for ll in visible:
            ...
            if ll["has_cursor"] and ll.get("cursor_pos") is not None:
                ...
                before = display[:cp]
                after = display[cp:]
                marker = EscapeCodes.CURSOR_MARKER if emit_marker else ""

                if after:
                    gs = _graphemes(after)
                    first_g = gs[0] if gs else ""
                    rest = after[len(first_g):]
                    cursor_ch = f"{EscapeCodes.INVERSE_ON}{first_g}{EscapeCodes.RESET}"
                    display = before + marker + cursor_ch + rest
                else:
                    cursor_ch = f"{EscapeCodes.INVERSE_ON} {EscapeCodes.RESET}"
    ...
    async def handle_input(self, data: str) -> None:
        ...
        # Bracketed paste
        if EscapeCodes.PASTE_START in data:
            self._is_in_paste = True
            self._paste_buffer = ""
            data = data.replace(EscapeCodes.PASTE_START, "")
        if self._is_in_paste:
            self._paste_buffer += data
            end_idx = self._paste_buffer.find(EscapeCodes.PASTE_END)
            if end_idx != -1:
